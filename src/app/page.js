...
async function getData() {
  ...
  let Slider = await prisma.blogs.findMany({ where: { type: "Slider" } });
  let Featured = await prisma.blogs.findMany({
    where: { type: "Featured" },
  });
  let Popular = await prisma.blogs.findMany({ where: { type: "Popular" } });
  let Latest = await prisma.blogs.findMany({
    orderBy: { id: "desc" },
    take: 12,
  });
